...
function startTimer() {
    ...
    intervalId = setInterval(() => {
        if (timer <= 0) {
            ...
            timerDisplay.classList.add('flash'); // Yanıp sönme animasyonunu başlat
            playEndSound(); // Ses dosyasını çal ve titreşimi başlat
            return;
        }
        timer--;
        updateTimer();
...
function stopTimer() {
    ...
    isRunning = false;
    startButton.disabled = false;
    timerDisplay.classList.remove('flash'); // Yanıp sönme animasyonunu kaldır
}
